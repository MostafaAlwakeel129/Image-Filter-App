#include <opencv2/opencv.hpp>
#include <opencv2/imgproc.hpp>
#include <vector>
#include <string>

using namespace cv;
using namespace std;

/* =========================================================
   TYPES
   ========================================================= */

struct ImageStats {
    float mean;
    float stddev;
    float min_val;
    float max_val;
};

/* =========================================================
   HISTOGRAM COMPUTATION
   ========================================================= */

// Raw bin counts (256 bins) for a grayscale image
vector<float> compute_histogram(const Mat& gray)
{
    CV_Assert(gray.type() == CV_8UC1);

    vector<float> hist(256, 0.0f);
    for (int y = 0; y < gray.rows; ++y) {
        const uchar* row = gray.ptr<uchar>(y);
        for (int x = 0; x < gray.cols; ++x)
            hist[row[x]]++;
    }
    return hist;
}

// Raw bin counts per channel — order is BGR (OpenCV native), not RGB
vector<vector<float>> compute_bgr_histograms(const Mat& color)
{
    CV_Assert(color.type() == CV_8UC3);

    vector<vector<float>> histograms(3, vector<float>(256, 0.0f));
    for (int y = 0; y < color.rows; ++y) {
        const Vec3b* row = color.ptr<Vec3b>(y);
        for (int x = 0; x < color.cols; ++x) {
            histograms[0][row[x][0]]++;   // B
            histograms[1][row[x][1]]++;   // G
            histograms[2][row[x][2]]++;   // R
        }
    }
    return histograms;
}

/* =========================================================
   CDF / PDF
   ========================================================= */

// Cumulative distribution function, normalised to [0, 1]
vector<float> compute_cdf(const vector<float>& hist)
{
    vector<float> cdf(256, 0.0f);
    cdf[0] = hist[0];
    for (int i = 1; i < 256; ++i)
        cdf[i] = cdf[i - 1] + hist[i];

    float total = cdf[255];
    if (total > 0)
        for (float& v : cdf)
            v /= total;

    return cdf;
}

// Probability density function (normalised histogram)
vector<float> compute_pdf(const vector<float>& hist)
{
    float total = 0.0f;
    for (float v : hist) total += v;

    vector<float> pdf(256, 0.0f);
    if (total > 0)
        for (int i = 0; i < 256; ++i)
            pdf[i] = hist[i] / total;

    return pdf;
}

/* =========================================================
   ALL-IN-ONE HELPERS
   Returns { hist, cdf, pdf } in one call to avoid redundant
   histogram traversals when the caller needs all three.
   ========================================================= */

// Grayscale: returns [hist, cdf, pdf]
vector<vector<float>> get_gray_histogram_and_cdf(const Mat& gray)
{
    auto hist = compute_histogram(gray);
    return { hist, compute_cdf(hist), compute_pdf(hist) };
}

// Color: returns [[B_hist, B_cdf, B_pdf], [G_...], [R_...]]
vector<vector<vector<float>>> get_bgr_histograms_and_cdfs(const Mat& color)
{
    auto hists = compute_bgr_histograms(color);
    vector<vector<vector<float>>> result;
    result.reserve(3);
    for (const auto& hist : hists)
        result.push_back({ hist, compute_cdf(hist), compute_pdf(hist) });
    return result;
}

/* =========================================================
   HISTOGRAM EQUALIZATION
   ========================================================= */

Mat equalize_image(const Mat& gray)
{
    CV_Assert(gray.type() == CV_8UC1);
    Mat equalized;
    equalizeHist(gray, equalized);
    return equalized;
}

Mat equalize_bgr(const Mat& color)
{
    CV_Assert(color.type() == CV_8UC3);
    vector<Mat> channels(3);
    split(color, channels);
    for (Mat& ch : channels)
        equalizeHist(ch, ch);
    Mat result;
    merge(channels, result);
    return result;
}

/* =========================================================
   NORMALIZATION
   ========================================================= */

Mat normalize_image(const Mat& gray)
{
    CV_Assert(gray.type() == CV_8UC1);
    Mat normalized;
    normalize(gray, normalized, 0, 255, NORM_MINMAX);
    return normalized;
}

Mat normalize_bgr(const Mat& color)
{
    CV_Assert(color.type() == CV_8UC3);
    vector<Mat> channels(3);
    split(color, channels);
    for (Mat& ch : channels)
        normalize(ch, ch, 0, 255, NORM_MINMAX);
    Mat result;
    merge(channels, result);
    return result;
}

/* =========================================================
   COLOR CONVERSION
   ========================================================= */

Mat color_to_gray(const Mat& color)
{
    CV_Assert(color.type() == CV_8UC3);
    Mat gray;
    cvtColor(color, gray, COLOR_BGR2GRAY);
    return gray;
}

/* =========================================================
   IMAGE STATISTICS
   ========================================================= */

ImageStats compute_stats(const Mat& gray)
{
    CV_Assert(gray.type() == CV_8UC1);

    Scalar mean, stddev;
    meanStdDev(gray, mean, stddev);

    double min_val, max_val;
    minMaxLoc(gray, &min_val, &max_val);

    return {
        static_cast<float>(mean[0]),
        static_cast<float>(stddev[0]),
        static_cast<float>(min_val),
        static_cast<float>(max_val)
    };
}

/* =========================================================
   CUSTOM MAPPING CURVE (LUT)
   ========================================================= */

Mat apply_mapping_curve(const Mat& gray, const vector<float>& mapping)
{
    CV_Assert(gray.type() == CV_8UC1);
    CV_Assert(mapping.size() == 256);

    Mat result(gray.size(), CV_8UC1);
    for (int y = 0; y < gray.rows; ++y) {
        const uchar* src = gray.ptr<uchar>(y);
        uchar*       dst = result.ptr<uchar>(y);
        for (int x = 0; x < gray.cols; ++x)
            dst[x] = static_cast<uchar>(
                std::min(255.0f, std::max(0.0f, mapping[src[x]])));
    }
    return result;
}